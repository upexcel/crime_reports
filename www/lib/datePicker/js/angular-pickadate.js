            ...
            .factory('pickadateModalBindings', ['$window', '$document', function($window, $document) {
                    ...
                    return function(scope, element, rootNode) {
                        ...
                        $document.on('click', function(e) {
                            if (isDescendant(rootNode, e.target) || e.target === element[0])
                                return;
                            togglePicker(false);
                        });
                    };
...
//                            $sce.trustAsHtml(i18n.t('prev')) +
                            ...
                            '</a>' +
                            '<a href="" class="pickadate-next" ng-hide="allowPrevMonth">' +
                            '</a>' +
                            '<div class="month">{{currentDate | date:"MMMM"}}</div>' +
                            '<a href="" class="pickadate-next" ng-click="changeMonth(1)" ng-show="allowNextMonth">' +
//                            $sce.trustAsHtml(i18n.t('next')) +
                            '<i  class="ion-android-arrow-forward"></i>' +
                            '</a>' +
                            '<a href="" class="pickadate-next" ng-hide="allowNextMonth">' +
                            '</a>' +
                            '</div>' +
                            '<div class="head-date">{{currentDate |date:"dd"}}</div>' +
                            '<div class="head-year">{{currentDate | date:"yyyy"}}</div>' +
                            // '<a href="" class="pickadate-prev" ng-click="changeMonth(-1)" ng-show="allowPrevMonth">' +
                            //   $sce.trustAsHtml(i18n.t('prev')) +
                            // '</a>' +
                            // '<a href="" class="pickadate-next" ng-click="changeMonth(1)" ng-show="allowNextMonth">' +
                    ...
                    return {
                        ...
                        link: function(scope, element, attrs, ngModel) {
                            var allowMultiple = attrs.hasOwnProperty('multiple'),
                                    ...
                                    format = (attrs.format || 'MM-dd-yyyy').replace(/m/g, 'M'),
                            ...
                            scope.setDate = function(dateObj) {
                                ...
                                scope.selectedDate = selectedDates[0].date;
...
//                                alert(offset);
                            ...
                            function render() {
                                ...
                                scope.dates = map(dates, function(date) {
                                    ...
                                    if (date.today)
                                        date.classNames.push('pickadate-today');
                                    if (date.disabled)
                                        date.classNames.push('pickadate-unavailable');
